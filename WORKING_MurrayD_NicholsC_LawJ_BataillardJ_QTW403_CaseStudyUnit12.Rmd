...
In order to investigate the relationship between lambda and kappa in our model, we carry out a simulation study, fixing kappa, which represents the parameter for the lifetime of each job, to be one. We run 400 Monte Carlo simulations to determine the number of child jobs created using each of six values of lambda, which represents the rate parameter for the number of offspring for each parent. We then run another set of simulations where kappa is not equal to one, however, the ratio of lambda to kappa is equal to one of the lambda values from the earlier simulation where kappa was equal to one. This allows us to analyze whether the relationship between these two parameters are important or if one or more parameters dominates the longevity of our queueing process.
