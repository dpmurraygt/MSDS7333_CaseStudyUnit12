...
Another example of a stochastic process is the Poisson process which we use in this paper to estimate the number and run time of jobs in a queueing process. It is used in instances when we need to count the number of occurrences of an event that happens at random. In practice, (4) the Poisson process is used to model the number of car accidents at a site, the location of users in a wireless network, the request for individual documents on a web server and the outbreak of wars. 

The Poisson process with rate $\lambda$ is defined as (4):
Let $\lambda$ > 0 be fixed. The counting process {N(t), t $\elementof$ [0,???)} is called a Poisson process with rate $\lambda$ if all the following conditions hold:
1.	N(0) = 0;
...
```{r dist_comparisons, echo=FALSE, include=TRUE, fig.height=6, fig.width=8.5}
...
## needs viz and more explanation, code done for sim
...
We selected several values for lambda and kappa with matching ratios to determing first in the test case to see the overall patterns that would emerge from the changes, and then the simulation case, iterating over 500 times to provide a representative sample.  Our first attempt was to change the lambda while keeping kappa constant at 1 as shown below.

```{r kappa_constant_1, echo=FALSE, include=TRUE, eval=FALSE, fig.height=8, fig.width=8.5}

lambdas = c(seq(.2, 2, by = 0.2))
kappas = c(1)
...
logUQkids = sapply(mcTrialOutput, function(x) 
...
library(scatterplot3d)
sdp = scatterplot3d(x = trialLambdas, y = trialKappas, 
...
```
The three-dimensional visualization attempts to illustrate the relationship between values of Kappa, Lambda, and the upper quartile's value of count of offspring - with a value of lambda=0, kappa=10 producing the lowest count of upper quartile offspring, and larger values of lamda producing higher upper quartile offspring.

```{r}
#try to make this into a GGplot faceted viz
str(mcOutputK1)
...
while (member <= totalMembers) {
  ...
  tmp %<>% mutate(lambda = lambdas[member], kappa = kappa)
...
AllExperiments %>% ggplot(aes(x=Offspring, Generations)) + geom_point(alpha=0.2) + facet_wrap( ~ lambda, ncol=5) + scale_x_log10(labels=scales::comma) + geom_jitter() + ggtitle("Kappa = 1, Lambda As Shown On Facet Label")

```
The visualizations show a similar  effect - smaller values of lambda allow for fewer numbers of offspring and fewer generations to be produced than slightly larger values of lambda produce.  The effect tapers off for generations above lambda = 1.0, with fewer generations, but more offspring produced for values of lambda above 1.2.

With the question of interest asking whether the ratios, or the absolute values of the two parameters forming the distribution of values, we set about to discover whether the ratios, or the absolute values, were the primary input to the outcomes.  

We tested kappa at a constant at 2, with keeping the ratios used the same as in the this example - starting at 0.4, and increasing by 0.4 unitl reaching maximum value of lambda=4. 

```{r kappa_constant_2, echo=FALSE, include=TRUE, eval=FALSE, fig.height=8, fig.width=8.5}
# setting simulation values for kappa and lambda, where kappa != to a constant of 1
# book calls for lambda / constant, so simply double to keep ratios the same, test to see if we get
# similar results visually
...
# testing to see if the ratio keeps these simulations roughly constant or if they differ
# expect constant
...
set.seed(1)
# simulation with doubled params
...
while (member <= totalMembers) {
  ...
  tmp %<>% mutate(lambda = lambdas2[member], kappa = kappa)
...
```{r fig.width=10, fig.height=5}
ModifiedParameters %>% ggplot(aes(x=Offspring, Generations)) + geom_point(alpha=0.2) + facet_wrap( ~ lambda, ncol=5) + scale_x_log10(labels=scales::comma) + geom_jitter() + ggtitle("Kappa = 2, Lambda As Shown On Facet Label")
```
A comparison of the visualization of the two sets of parameters - the first with kappa=1 and lambda values from 0.2 to 2, incremented by 0.2, and the second, with kappa=2 and lambda values from 0.4 to 4, show identical visualizations of the distributions as their equivalent for the opposie value of kappa.  This would lead us to conclude that the ratio of the two values, lambda and kappa, are the determinants of the number of generations for a given number of offspring, and not the absolute values of the two variables.  The process shows similar clusters produced - a number of examples of the product of the Monte Carlo process producing up to 10 offspring, and a number of iterations on larger values of lambda producing more than 1,000 offspring.
...
8. Smid, J. et al. "Strengths and weaknesses of Monte Carlo simulation models and Bayesian belief networks in microbial risk assessment." ScienceDirect, 30 May 2010. https://www.sciencedirect.com/science/article/pii/S0168160509006680

9. Heffernan, R. "Monte Carlo and Manufacturing." Quality Magazine, 1 April 2014.
