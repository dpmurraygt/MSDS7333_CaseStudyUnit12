---
title: "Initial Look at the Code"
output: html_notebook
---

```{r}

#creates a vector of 6000 items, each one that is composed of sum of a 3 value random exponential distribution of rate 1
...
#vector from 0.1 to 1 by tenths, and then 2 to 7 by 1's
rates = c(seq(0.1, 1, by = 0.1), seq(2, 7, by = 1))

#similar to empirical, this one creates a variable rate 
samples = lapply(rates, function(r) {
             replicate(6000, sum(rexp(n = 3, rate = r))) })

#create a PDF that has visualization of multiple different outcomes of cumulative distribution versus values of x
...
#create a value d0 from exponential distribution with a rate of 0.3
kappa = 0.3
d0 = rexp(1, rate = kappa)
d0

#create a value birth1 from exponential distribution with a rate of 0.5
lambda = 0.5
birth1 = rexp(1, rate = lambda)
birth1

#recursively building itime
...
#calculate the cumulative sum (same length as vector) of itime
cumsum(itime)

itime = c(itime, rexp(1, rate = lambda))

#btime is comprised of the cumsum of itimes
btime = cumsum(itime)
btime

#keep only btime entries that are < d0
